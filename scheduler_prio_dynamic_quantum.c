...
struct proc * scheduler(struct proc * current)
{
    ...
    if (current != NULL)
    {
        ...
        {
            case READY:
                // Verifica quanto tempo o processo usou na última vez na CPU
                if (current->process_time >= (QUANTUM / 2))
                {
                    // Usou mais de 50% do quantum → volta para fila 1 (ready)
                    current->queue = 0;
                    enqueue(ready, current);
                }
                else
                {
                    // Usou menos de 50% → vai para fila 2 (ready2)
                    current->queue = 1;
            ...
            case BLOCKED:
                // Saiu para E/S → volta para fila 1
                current->queue = 0;
                enqueue(ready, current);
